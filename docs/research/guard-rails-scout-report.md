...
**Why #3:** RDNA already has `recommended` (warn), `internals` (warn, no component enforcement), and `recommended-strict` (error, not activated). Carbon's explicit `light-touch` tier provides a better on-ramp for new consumers. The key insight is that the tiers differ in *exception tolerance*, not just severity.
