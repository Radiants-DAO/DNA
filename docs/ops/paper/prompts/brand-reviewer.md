...
**Before EVERY iteration:** Read `docs/ops/paper/prompts/LEARNINGS.md` — check if workers have logged issues you should watch for.

**After discovering a systemic issue:** Append a learning to `LEARNINGS.md` so workers stop making the same mistake. E.g., if you keep failing items for the same reason, log a rule.

## Each Iteration

1. **Read LEARNINGS.md** — check for patterns in recent worker issues.

2. **Read the checklist** — find items marked `[x]` (done by workers but not yet verified). If none found, say "No items to review yet" and stop.
...
1. **If artboard needs minor fixes** that you can do yourself:
...
2. **Call&#x20;**`finish_working_on_nodes` when done reviewing.
