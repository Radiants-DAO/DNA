...
export function Web3ActionBar({
  ...
  return (
    <div data-rdna="web3actionbar" className={`flex items-center justify-between gap-4 px-2 py-2 bg-page border-t border-line shrink-0 ${className}`.trim()}>
      ...
      <div className="flex gap-2 items-center">
        {!isConnected ? (
          ...
          <>
            ...
            {walletAddress && (
              <span className="font-mono text-sm text-main">{formatAddress(walletAddress)}</span>
            )}
            {/* Disconnect button */}
            {onDisconnect && (
              <Button
