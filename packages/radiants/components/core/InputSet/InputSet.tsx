...
function Root({ children, className = '', disabled }: RootProps): React.ReactNode {
  return (
    <BaseFieldset.Root
      data-rdna="input-set"
      disabled={disabled}
      className={`rounded-xs border border-line p-4 transition-shadow duration-150 focus-within:outline focus-within:outline-2 focus-within:outline-focus ${className}`.trim()}
    >
      {children}
    </BaseFieldset.Root>
  );
