...
const variantStyles: Record<CountdownVariant, {
  ...
  default: {
    container: 'p-4 bg-warm-cloud border border-black rounded-md',
    label: 'font-mono text-xs text-black/60 mb-2',
    timer: 'flex items-center justify-center gap-1',
    segment: 'flex flex-col items-center min-w-[3rem]',
    value: 'font-joystix text-2xl text-black tabular-nums',
    unit: 'font-mono text-2xs text-black/50 uppercase mt-0.5',
    separator: 'font-joystix text-xl text-black/40 self-start mt-1',
  },
  compact: {
    container: 'px-3 py-2 bg-warm-cloud border border-black rounded-sm',
    label: 'font-mono text-2xs text-black/60 mb-1',
    timer: 'flex items-center justify-center gap-0.5',
    segment: 'flex flex-col items-center min-w-[2rem]',
    value: 'font-joystix text-sm text-black tabular-nums',
    unit: 'font-mono text-2xs text-black/50 uppercase',
    separator: 'font-joystix text-sm text-black/40 self-start',
  },
  large: {
    container: 'p-6 bg-warm-cloud border border-black rounded-md shadow-[2px_2px_0_0_var(--color-black)]',
    label: 'font-mono text-sm text-black/60 mb-3',
    timer: 'flex items-center justify-center gap-2',
    segment: 'flex flex-col items-center min-w-[4rem] bg-cream border border-black rounded-sm px-3 py-2',
    value: 'font-joystix text-3xl text-black tabular-nums',
    unit: 'font-mono text-xs text-black/50 uppercase mt-1',
    separator: 'font-joystix text-2xl text-black/40 self-center',
  },
};
  ...
  if (status === 'ended') {
    return (
      <div className={`${baseStyles} ${styles.container} ${className}`}>
        ...
        <p className={`font-joystix ${variant === 'large' ? 'text-xl' : variant === 'compact' ? 'text-sm' : 'text-lg'} text-black/60`}>
          {endedMessage}
        </p>
      </div>
    );
