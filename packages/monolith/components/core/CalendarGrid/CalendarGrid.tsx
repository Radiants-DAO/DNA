...
export function CalendarGrid({
  ...
  return (
    <div className={`cal-month ${className}`.trim()}>
      <div className="cal-month-header panel-label">
        {MONTH_NAMES[month]} {year}
      </div>
      <div className="cal-grid">
        {DAY_NAMES.map((d) => (
          <div key={d} className="cal-cell cal-cell--header panel-muted">
            {d}
          </div>
        ))}
        {cells.map((day, i) => {
          ...
          return (
            <div
              key={key}
              className={`cal-cell cal-cell--day${isToday ? ' cal-cell--today' : ''}${isPast ? ' cal-cell--past' : ''}${hasContent ? ' cal-cell--has-event' : ''}${specialClass}${isSelected ? ' cal-cell--selected' : ''}`}
            ...
            >
              <span className={`cal-date${specialEvent ? ' cal-date--bold' : ''}`}>
                {day}
              </span>
              {postCount > 0 && (
