...
1. Read `ops/paper-loop-prompts/LEARNINGS.md` — apply any rules that affect your current item.

**After hitting a gotcha:**
1. Fix the issue in your current work.
2. Append a learning to `LEARNINGS.md` using the format documented there.
3. Categorize it: `PAPER-MCP`, `FIGMA-MCP`, `CSS`, `FONTS`, `IMAGES`, `PATTERNS`, `LAYOUT`, `WORKFLOW`.
4. Include a one-line **Rule** that future agents can follow without understanding the full context.

**What counts as a gotcha:**
- A Paper tool call that fails or produces unexpected results
- A CSS property that Paper ignores or renders differently than expected
- A font that doesn't render correctly
- An image URL pattern that breaks
- A layout technique that doesn't work in Paper
- Any workaround you had to invent

**Do NOT log:** routine work, expected behavior, or things already documented in the prompt.

## Each Iteration

1. **Read LEARNINGS.md** — check for any rules relevant to your next item.

2. **Read the checklist** — find the FIRST item with `[ ]` status (unchecked). If no unchecked items remain from the top half (sections 1-5), stop and say "Top-down worker complete."
...
7. **Log learnings** — if you hit any gotcha, append to `ops/paper-loop-prompts/LEARNINGS.md`.

8. **Call `finish_working_on_nodes`** to release the artboard.
