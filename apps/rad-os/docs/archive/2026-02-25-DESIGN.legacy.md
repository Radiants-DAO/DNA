...
This file is written to be portable across implementation domains (web, native, desktop, CLI-like surfaces). The design system has three contract layers:

1. Philosophy contract (Section 1): emotional intent and design invariants.
2. Token and behavior contract (Sections 2-9): named primitives and rules.
3. Product contract (Part 2+): RadOS-specific patterns and constraints.

Any implementation conforms if it preserves all of the following:

- Semantic token names and role mappings (even if values are adapted per platform).
- Sun/Moon mood semantics (not just color inversion).
- Environment > Surface > Chrome hierarchy and priority.
- Motion, interaction, and accessibility guardrails defined in this file.

Platform adapters MAY change syntax (CSS vars, Swift enums, Kotlin objects, etc.), but MUST NOT change intent.
