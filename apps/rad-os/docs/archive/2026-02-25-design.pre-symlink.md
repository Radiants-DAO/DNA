...
- [ ] Uses RDNA semantic token names and role mappings (or exact aliases) instead of raw primitives in component code.
...
> **Deprecated aliases:** `--color-black` -> `--color-ink`, `--color-white` -> `--color-pure-white`, `--color-green` -> `--color-mint`. Keep aliases only for migration windows.
...
| `--color-depth` | ink (5% opacity) | cream (5% opacity) |
| `--color-hover` | ink (10% opacity) | cream (10% opacity) |

#### Overlay Tokens

| Token | Sun Mode | Moon Mode |
|-------|----------|-----------|
| `--color-hover` | ink (5% opacity) | cream (8% opacity) |
| `--color-active` | ink (10% opacity) | cream (12% opacity) |
...
| `--color-sub` | ink (85% opacity) | cream |
