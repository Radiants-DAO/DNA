  ...
  return (
    <div className="max-w-xl mx-auto p-8">
      ...
      <header className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <img
            ...
            className="w-8 h-8"
          />
          <h1 className="text-2xl font-semibold text-content-primary">RadMark Settings</h1>
        </div>
      ...
      <div className="space-y-6">
        ...
        <section>
          ...
          <div className="card-radiants p-4">
            <div className="flex items-start justify-between">
              <div>
                <span className="text-sm font-medium text-content-primary">Obsidian Vault</span>
                <span className="block text-xs text-content-muted mt-1 mb-3">
                  Connect to your Obsidian vault to save bookmarks as JSON files
                </span>
              </div>
              {vaultStatus === 'connected' && (
                <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-status-success text-content-primary rounded border border-edge-primary">
                  Connected
                </span>
              )}
            </div>
            ...
            {!fileSystemSupported ? (
              <div className="p-3 bg-surface-muted rounded border border-edge-muted">
              ...
              <div className="text-sm text-content-muted">Checking vault access...</div>
            ) : vaultStatus === 'connected' ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2 p-3 bg-surface-muted rounded border border-edge-muted">
                  <svg className="w-4 h-4 text-status-success flex-shrink-0" viewBox="0 0 16 16" fill="currentColor">
                  ...
                  <span className="text-sm text-content-primary font-medium truncate">{vaultName}</span>
                  <span className="text-xs text-content-muted flex-shrink-0">/.pending/bookmarks.json</span>
                </div>
                <button
              ...
              <div className="space-y-3">
                <p className="text-xs text-content-muted">
                  ...
                  <code className="mx-1 px-1 py-0.5 bg-surface-muted rounded text-xs">.pending</code>
          ...
          <div className="card-radiants p-4 mt-3">
            <label className="block">
              <span className="text-sm font-medium text-content-primary">Manual Vault Path</span>
              <span className="block text-xs text-content-muted mt-1 mb-2">
          ...
          <div className="card-radiants p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <span className="text-sm font-medium text-content-primary">Bookmark Shortcut</span>
                <span className="block text-xs text-content-muted mt-1">
                  Press this key when a tweet is focused to toggle RadMark bookmark.
                  Use <kbd className="px-1.5 py-0.5 bg-surface-muted border border-edge-muted rounded text-xs font-mono">Alt</kbd> + key to open context popup.
                </span>
              </div>
              <div className="flex items-center gap-2">
                {isCapturingShortcut ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 bg-surface-muted border border-edge-primary rounded hover:bg-surface-elevated transition-colors"
                    title="Click to change shortcut"
                  >
                    <kbd className="px-2 py-1 bg-action-primary text-content-primary border border-edge-primary rounded font-mono text-sm font-bold min-w-[32px] text-center">
          ...
          <div className="card-radiants p-4">
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                ...
                className="mt-0.5 w-4 h-4 accent-action-primary cursor-pointer"
              />
              <span>
                <span className="text-sm font-medium text-content-primary">Clipboard Fallback</span>
                <span className="block text-xs text-content-muted mt-1">
        ...
        {/* Danger Zone */}
        <section>
          <h2 className="text-xs font-medium text-status-error uppercase tracking-wide mb-3">
            Danger Zone
          </h2>
          <div className="card-radiants p-4 border-status-error">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <span className="text-sm font-medium text-content-primary">Clear All Pending Bookmarks</span>
                <span className="block text-xs text-content-muted mt-1">
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded border transition-colors ${
                  confirmClear
                    ? 'bg-status-error text-white border-status-error hover:opacity-90'
                    : pendingCount === 0
                    ? 'bg-surface-muted text-content-muted border-edge-muted cursor-not-allowed'
                    : 'bg-surface-primary text-status-error border-status-error hover:bg-status-error hover:text-white'
                }`}
              >
            ...
            {confirmClear && (
              <div className="mt-3 p-3 bg-status-error/10 border border-status-error rounded text-xs text-status-error">
        ...
        <section className="pt-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs text-content-muted hover:text-content-primary hover:underline"
            >
              Reset to defaults
            </button>
          </div>
        ...
        {/* About Section */}
        <section className="pt-4 mt-4 border-t border-edge-muted">
