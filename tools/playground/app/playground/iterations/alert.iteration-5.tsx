...
// ============================================================================
// Styles — Variation 1: Elevated Card
// Elevated surface with shadow-raised, rounded-md corners, and more generous
// internal spacing. Feels like a panel surfaced from the desktop.
// ============================================================================

export const alertVariants = cva(
  'p-5 border rounded-md shadow-raised',
  {
    variants: {
      variant: {
...
function Title({ children, className = '' }: AlertChildProps): React.ReactElement {
  return (
    <h4 className={`text-base font-heading uppercase tracking-tight leading-none mb-1.5 text-balance text-content-heading ${className}`}>
      {children}
    </h4>
  );
}
...
function Close({ children, onClick, className = '' }: AlertCloseProps): React.ReactElement {
  return (
    <button
      onClick={onClick}
      className={`flex-shrink-0 p-1.5 rounded-sm hover:bg-hover-overlay transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-edge-focus focus-visible:ring-offset-1 ${className}`}
