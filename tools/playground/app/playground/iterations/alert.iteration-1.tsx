...
export const alertVariants = cva(
  'p-4 border border-line rounded-sm shadow-raised',
  {
    variants: {
      variant: {
        default: 'bg-accent text-main',
        success: 'bg-success text-main',
        warning: 'bg-warning text-main',
        error: 'bg-danger text-main',
        info: 'bg-link text-main',
      },
    },
    defaultVariants: {
      variant: 'default',
...
function Close({ children, onClick, className = '' }: AlertCloseProps): React.ReactElement {
  return (
    <button
      onClick={onClick}
      className={`flex-shrink-0 p-1 rounded-sm hover:bg-page/20 transition-colors duration-fast cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-1 ${className}`}
