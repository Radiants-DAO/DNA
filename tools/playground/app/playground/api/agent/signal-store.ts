export type PlaygroundSignalEvent =
  ...
  | { type: "capture-request"; previewUrl: string; requestId: string };
...
class SignalStore {
  ...
  captureRequest(previewUrl: string, requestId: string): void {
    this.emit({ type: "capture-request", previewUrl, requestId });
  }
