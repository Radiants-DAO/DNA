 ...
 *
 * Based on robot.co canvas implementation patterns.
 *
 * @example
 * ```tsx
 * const { containerProps, registerDraggable, scale, state } = useCanvasGestures({
 *   enablePan: true,
 *   enableZoom: true,
 *   minScale: 0.25,
 *   maxScale: 4,
 ...
 * return (
 *   <div {...containerProps}>
 *     <CanvasNode ref={(el) => registerDraggable('node-1', el)} />
 *   </div>
 * );
 * ```
 */
export function useCanvasGestures(
  ...
  const onMouseDown = useCallback(
    ...
    [opts.enableDrag, opts.enablePan, opts.panModifier, findDraggableAtPoint, startGesture]
  );
  ...
  const onTouchStart = useCallback(
    ...
    [opts.enableDrag, opts.enablePan, opts.enableZoom, findDraggableAtPoint, startGesture]
  );
