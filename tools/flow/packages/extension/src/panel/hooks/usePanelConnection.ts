/**
 * usePanelConnection — shared connection logic for DevTools panel and Side Panel.
 *
 * Extracts contentBridge initialization, message handling, Zustand sync,
 * and mutation/text-edit bridges from Panel.tsx so both surfaces can reuse it.
 *
 * The only difference between DevTools and Side Panel is the tabId source:
 * - DevTools: chrome.devtools.inspectedWindow.tabId (sync)
 * - Side Panel: useActiveTabId() (async, changes on tab switch)
 */
...
import { initContentBridge, disconnectContentBridge, onContentMessage } from '../api/contentBridge';
import {
  isRuntimeMessagingError,
  safePortPostMessage,
} from '../../utils/runtimeSafety';
import type { InspectionContextValue } from '../context/InspectionContext';
...
export function usePanelConnection(tabId: number | null): InspectionContextValue {
  ...
  // Content bridge connection — reconnects when tabId changes
  useEffect(() => {
