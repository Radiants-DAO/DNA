...
import type { Vector2D, VelocitySample } from "../types/canvas";

/**
 * Physics configuration
 */
export interface PhysicsConfig {
  /** Base friction coefficient (0-1) */
  baseFriction: number;
  /** High-speed friction coefficient (0-1) */
  highSpeedFriction: number;
  /** Bounce damping factor (0-1) */
  bounceDamping: number;
  /** Minimum velocity threshold (px/frame) */
  minVelocity: number;
  /** Maximum velocity (px/frame) */
  maxVelocity: number;
  /** Number of velocity samples to average */
  velocitySampleCount: number;
  /** Max age of velocity samples (ms) */
...
/**
 * Options for useCanvasPhysics hook
 */
export interface UseCanvasPhysicsOptions {
  /** Custom physics configuration */
  config?: Partial<PhysicsConfig>;
  /** Content bounds for bounce detection */
  bounds?: { width: number; height: number };
  /** Enable bounce at boundaries */
...
/**
 * Result from useCanvasPhysics hook
 */
