/**
 * ComponentCanvas - Spatial canvas view for component schemas
 *
 * Features:
...
): { width: number; height: number; right: number } {
  if (nodes.length === 0) {
...
export function ComponentCanvas(): React.ReactElement {
  ...
  const connectionVisibility = useAppStore(
    (s) => s.componentConnectionVisibility
  );
  ...
  const toggleComponentNodeSelection = useAppStore(
    (s) => s.toggleComponentNodeSelection
  );
  ...
  // Track container size and rect for MarqueeSelection
  useEffect(() => {
