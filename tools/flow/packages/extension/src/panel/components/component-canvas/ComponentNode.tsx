/**
 * ComponentNode - A component schema card on the canvas
 *
 * Ported from Flow 0.
 */
...
export function ComponentNode({
  ...
  const propCount = useMemo(
    () => Object.keys(schema.props).length,
    [schema.props]
  );
  const slotCount = useMemo(
    () => getSlotsArray(schema.slots).length,
    [schema.slots]
  );
  ...
  return (
    ...
    >
      ...
      >
        <Component
          size={16}
          className="text-[rgba(255,255,255,0.6)] flex-shrink-0"
        />
      ...
      <div className="flex-1 px-3 py-2 space-y-1.5 overflow-hidden">
        ...
        <div className="flex items-start gap-2 text-xs">
          <Sliders
            size={12}
            className="text-[rgba(255,255,255,0.4)] flex-shrink-0 mt-0.5"
          />
        ...
        <div className="flex items-start gap-2 text-xs">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="text-[rgba(255,255,255,0.8)]">{exampleCount}</span>
          </div>
        </div>
