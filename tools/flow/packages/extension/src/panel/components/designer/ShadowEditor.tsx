...
function ShadowLayer({
  ...
  return (
    <div className="bg-neutral-800/30 rounded-lg border border-neutral-700">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-neutral-700">
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="flex items-center gap-2 text-xs text-neutral-200 hover:text-blue-400 transition-colors"
          disabled={disabled}
        >
          <svg
            width="12"
          ...
          {shadow.position === "inset" && (
            <span className="text-[10px] text-neutral-400 bg-neutral-700/50 px-1.5 rounded">inset</span>
          )}
        </button>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-1 rounded hover:bg-neutral-800/50 text-neutral-400 hover:text-neutral-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            title="Move up"
          >
          ...
          <button
            onClick={onRemove}
            disabled={disabled}
            className="p-1 rounded hover:bg-red-500/20 text-neutral-400 hover:text-red-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            title="Remove shadow"
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {isExpanded && (
        <div className="p-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-neutral-400 uppercase tracking-wider block mb-1">
                X Offset {hasVarOffset && <span className="text-blue-400">(var)</span>}
              </label>
              <div className="flex gap-1">
                <input
                  ...
                  disabled={disabled}
                  className="flex-1 h-7 bg-neutral-800/50 border border-neutral-700 rounded-md px-2 text-xs text-neutral-200 font-mono disabled:opacity-50"
                />
                <span className="h-7 flex items-center px-1.5 text-[10px] text-neutral-400 bg-neutral-800/30 rounded-md">
                  px
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-neutral-400 uppercase tracking-wider block mb-1">
                Blur {hasVarBlur && <span className="text-blue-400">(var)</span>}
              </label>
              <div className="flex gap-1">
                <input
                  type="number"
            ...
            <div>
              <label className="text-[10px] text-neutral-400 uppercase tracking-wider block mb-1">
                Spread {hasVarSpread && <span className="text-blue-400">(var)</span>}
              </label>
              <div className="flex gap-1">
                <input
                  type="number"
          ...
          <div>
            <label className="text-[10px] text-neutral-400 uppercase tracking-wider block mb-1">
              Color {hasVarColor && <span className="text-blue-400">(var: {(shadow.color as VarValue).value})</span>}
            </label>
            <div className="flex gap-2">
              <input
                ...
                disabled={disabled}
                className="w-8 h-7 rounded border border-neutral-700 cursor-pointer disabled:opacity-50"
              />
              <input
                type="text"
                value={colorHex}
              ...
              <div className="flex items-center gap-1">
                <input
                  ...
                  disabled={disabled}
                  className="w-16 h-1.5 bg-neutral-700/50 rounded disabled:opacity-50"
                />
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs transition-colors ${
                shadow.position === "inset"
                  ...
                  : "bg-neutral-800/50 text-neutral-400 border border-neutral-700 hover:text-neutral-200"
              } disabled:opacity-50`}
            >
              Inset
            </button>
...
export function ShadowEditor({ value, onChange, disabled }: ShadowEditorProps) {
  ...
  return (
    <DogfoodBoundary name="ShadowEditor" file="designer/ShadowEditor.tsx" category="designer">
    <div className="space-y-3">
      {/* Preview */}
      <div className="flex justify-center p-4 bg-neutral-800/30 rounded-lg border border-neutral-700">
        <div
          className="w-20 h-16 bg-neutral-800 rounded-lg"
      ...
      <button
        ...
        disabled={disabled}
        className="w-full py-2 border border-dashed border-neutral-700 rounded-lg text-xs text-neutral-400 hover:text-neutral-200 hover:border-neutral-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5"
      >
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {previewCss && (
        <div className="bg-neutral-800/30 rounded-lg p-2">
          <div className="text-[10px] text-neutral-400 uppercase tracking-wider mb-1">CSS</div>
          <code className="text-[11px] text-neutral-200 font-mono break-all">{previewCss}</code>
        </div>
      )}
    </div>
    </DogfoodBoundary>
