...
function ColorStop({
  ...
  return (
    <div className="flex items-center gap-2 p-2 bg-neutral-800/30 rounded-lg">
      {/* Color swatch */}
      <input
        type="color"
        value={colorHex}
        ...
        disabled={disabled}
        className="w-7 h-7 rounded border border-neutral-700 cursor-pointer disabled:opacity-50"
      />
      ...
      <input
        ...
        onChange={(e) => handleColorChange(e.target.value)}
        disabled={disabled}
        className="w-20 h-7 bg-neutral-800/50 border border-neutral-700 rounded-md px-2 text-xs text-neutral-200 font-mono disabled:opacity-50"
      />
      ...
      <div className="flex items-center gap-1">
        <input
          ...
          disabled={disabled}
          className="w-12 h-1.5 bg-neutral-700/50 rounded disabled:opacity-50"
        />
      ...
      <div className="flex items-center gap-1">
        <input
          ...
          disabled={disabled}
          className="w-12 h-7 bg-neutral-800/50 border border-neutral-700 rounded-md px-2 text-xs text-neutral-200 font-mono text-center disabled:opacity-50"
        />
...
export function GradientEditor({ value, onChange, disabled }: GradientEditorProps) {
  ...
  return (
    <DogfoodBoundary name="GradientEditor" file="designer/GradientEditor.tsx" category="designer">
    <div className="space-y-3">
      {/* Preview bar */}
      <div
        ref={previewBarRef}
        onClick={handlePreviewClick}
        className="h-10 rounded-lg border border-neutral-700 cursor-crosshair relative overflow-hidden"
      ...
      {(value.type === "linear" || value.type === "conic") && (
        <div>
          ...
          <div className="flex items-center gap-2">
            {/* Angle dial */}
            <div className="relative w-10 h-10 rounded-full border border-neutral-700 bg-neutral-800/30">
              <div
                className="absolute top-1/2 left-1/2 w-4 h-0.5 bg-blue-500 origin-left rounded-full"
                style={{ transform: `translateY(-50%) rotate(${(value.angle ?? 90) - 90}deg)` }}
              />
              <div className="absolute top-1/2 left-1/2 w-1.5 h-1.5 bg-blue-500 rounded-full -translate-x-1/2 -translate-y-1/2" />
            </div>
            <input
              type="range"
              min="0"
              ...
              disabled={disabled}
              className="flex-1 h-1.5 bg-neutral-700/50 rounded disabled:opacity-50"
            />
            <div className="flex gap-1 items-center">
              <input
                ...
                disabled={disabled}
                className="w-14 h-7 bg-neutral-800/50 border border-neutral-700 rounded-md px-2 text-xs text-neutral-200 font-mono text-center disabled:opacity-50"
              />
              <span className="text-[10px] text-neutral-400">deg</span>
            </div>
          </div>
        </div>
      )}
