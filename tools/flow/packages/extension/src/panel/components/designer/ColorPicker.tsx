 ...
 *
 * Ported from Flow 0. Full-featured color picker with:
 * - Token suggestions and matching
 * - Multiple input modes (hex, rgb, hsl, oklch)
...
const RECENT_COLORS_KEY = "flow-recent-colors";
...
export function ColorPicker({
  ...
  return (
    <DogfoodBoundary name="ColorPicker" file="designer/ColorPicker.tsx" category="designer">
    <div
      ref={containerRef}
    ...
    >
      ...
      <div
        className={`
          ...
          ${isExpanded ? "border-blue-500 bg-blue-500/10" : "border-neutral-700 bg-neutral-800 hover:border-blue-500/50"}
        `}
        onClick={() => !disabled && setIsExpanded(!isExpanded)}
      >
        {/* Color Swatch */}
        <div
          className="w-6 h-6 rounded border border-neutral-700 flex-shrink-0"
          style={{
        ...
        <span className="flex-1 text-sm font-mono truncate text-neutral-200">
      ...
      {/* Expanded Picker */}
      {isExpanded && (
        <div className="absolute top-full left-0 right-0 mt-2 z-50 bg-neutral-800 border border-neutral-700 rounded-lg shadow-xl">
          {/* Saturation/Lightness Plane */}
...
function HueSlider({ value, onChange }: HueSliderProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      />
      <span className="text-xs text-neutral-400 w-8 text-right">{hsl.h}</span>
    </div>
  );
}
    ...
    <div>
        ...
        {(["hex", "rgb", "hsl", "oklch"] as InputMode[]).map((mode) => (
          <button
            ...
            className={`
              ...
              ${inputMode === mode ? "bg-blue-500 text-white" : "bg-neutral-800 text-neutral-400 hover:text-neutral-200"}
            `}
          >
            {mode}
          </button>
      ...
      {inputMode === "hex" && (
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            maxLength={6}
            className="flex-1 px-2 py-1.5 text-sm font-mono bg-neutral-800 border border-neutral-700 rounded focus:outline-none focus:border-blue-500"
          />
        </div>
      )}
        ...
        <div className="flex gap-2">
          {(["r", "g", "b"] as const).map((channel) => (
            <div key={channel} className="flex-1">
              <label className="text-[10px] text-neutral-400 uppercase block mb-0.5">
                {channel}
              </label>
              <input
                type="number"
                ...
                className="w-full px-2 py-1.5 text-sm font-mono bg-neutral-800 border border-neutral-700 rounded focus:outline-none focus:border-blue-500"
              />
            </div>
          ))}
        </div>
      ...
      {inputMode === "oklch" && (
        <div>
          ...
          {/* CSS output preview */}
          <div className="mt-2 text-[10px] text-neutral-400 font-mono truncate">
...
function EyedropperButton({ onColorPicked }: EyedropperButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className="flex items-center gap-2 px-3 py-1.5 text-sm text-neutral-400 hover:text-neutral-200 bg-neutral-800 border border-neutral-700 rounded hover:border-blue-500/50 transition-colors"
    >
    ...
    <div className="border-t border-neutral-700">
      <div className="px-3 py-2 border-b border-neutral-700">
        <input
          ...
          className="w-full px-2 py-1 text-xs bg-neutral-800 border border-neutral-700 rounded focus:outline-none focus:border-blue-500"
        />
      </div>
      <div className="p-2">
        <div className="text-[10px] text-neutral-400 uppercase tracking-wider mb-1 px-1">
          Tokens
        </div>
        <div className="flex flex-wrap gap-1">
          {filteredTokens.map((token) => (
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 rounded border border-neutral-700 hover:border-blue-500/50 bg-neutral-800 text-xs transition-colors"
              title={`${token.name}: ${token.value}`}
            >
              <div
                className="w-3 h-3 rounded-sm border border-neutral-700/50"
                style={{ backgroundColor: token.resolvedHex }}
              />
              <span className="truncate max-w-[80px] text-neutral-200">
                {token.name.replace("--", "")}
              </span>
            </button>
          ))}
      ...
      <div className="flex gap-1">
        {colors.map((hex, i) => (
          <button
            ...
            className="w-6 h-6 rounded border border-neutral-700 hover:border-blue-500 transition-colors"
