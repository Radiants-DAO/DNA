 ...
 *
 * Ported from Webstudio's outline components with adaptations for RadFlow:
...
export function Outline({
  ...
  return (
    <div
      ...
      data-radflow-canvas-tool="outline"
    >
      {/* Label */}
      {showLabel && label && (
        <div
...
function useElementRect(
  ...
  const updateRect = useCallback(() => {
    ...
    try {
      ...
      // Find element by selector or radflow-id
      let element: Element | null = null;
      if (selector.startsWith("[data-radflow-id=")) {
        element = iframeDoc.querySelector(selector);
      } else {
        // Try to find by radflow-id attribute value
        element = iframeDoc.querySelector(`[data-radflow-id="${selector}"]`);
      }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const element = iframeDoc.querySelector(`[data-radflow-id="${selector}"]`);
      if (!element) return;
