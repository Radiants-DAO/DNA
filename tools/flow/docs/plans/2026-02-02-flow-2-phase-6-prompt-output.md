...
**Architecture:** A PromptCompiler service aggregates data from all Zustand slices (mutations, annotations, text edits, designer changes, animation diffs, prompt builder steps) into spec-compliant markdown. The Prompt Builder UI provides a visual query builder with element slots and property dropdowns, filtered through language adapters (CSS/Tailwind/Figma). Session state persists via chrome.storage APIs with per-tab isolation.
...
import type { Annotation } from '@flow/shared/types/annotations';
import type { TextEdit } from '@flow/shared/types/textEdits';
import type { DesignerChange } from '@flow/shared/types/designer';
import type { AnimationDiff } from '@flow/shared/types/animations';
import type { PromptStep } from '@flow/shared/types/promptBuilder';
...
**File:** `packages/extension/src/stores/slices/promptOutputSlice.ts`
...
**File:** `packages/extension/src/stores/slices/promptBuilderSlice.ts`

```typescript
import type { StateCreator } from 'zustand';
import type { AppState } from '../appStore';
import type { PromptStep, PromptVerb, LanguageAdapter } from '@flow/shared/types/promptBuilder';
...
**File:** `packages/extension/src/stores/appStore.ts`
...
```typescript
// Add to imports
import { createPromptOutputSlice, type PromptOutputSlice } from './slices/promptOutputSlice';
import { createPromptBuilderSlice, type PromptBuilderSlice } from './slices/promptBuilderSlice';

// Add to AppState type intersection
export type AppState = /* ...existing slices... */ & PromptOutputSlice & PromptBuilderSlice;
...
```typescript
import type { LanguageAdapter } from '@flow/shared/types/promptBuilder';
...
**File:** `packages/extension/src/components/panels/PromptBuilderPanel.tsx`

```tsx
import React from 'react';
import { useAppStore } from '../../stores/appStore';
import { translate, getDropdownOptions } from '../../services/languageAdapters';
import type { PromptVerb, LanguageAdapter } from '@flow/shared/types/promptBuilder';
...
**File:** `packages/extension/src/components/panels/ContextOutputPanel.tsx`
...
## Task 9: Comment/annotation mode — annotationSlice

**File:** `packages/extension/src/stores/slices/annotationSlice.ts`
...
**Commit:** `feat(flow2): add annotationSlice for comment mode`
...
import type { DesignerChange } from '@flow/shared/types/designer';
import type { AnimationDiff } from '@flow/shared/types/animations';
import type { PromptStep, LanguageAdapter } from '@flow/shared/types/promptBuilder';
...
**File:** `packages/extension/src/hooks/usePromptAutoCompile.ts`
...
**File:** `packages/extension/src/hooks/useSessionAutoSave.ts`
...
**File:** `packages/extension/src/hooks/useSessionRestore.ts`
...
## Task 18: Register annotationSlice in appStore + wire to content script

Add the annotation slice to the store and set up the content script message handler that triggers `addAnnotation` when the user submits annotation text.

Edit `packages/extension/src/stores/appStore.ts` to include:

```typescript
import { createAnnotationSlice, type AnnotationSlice } from './slices/annotationSlice';

// Add to AppState type
export type AppState = /* ...existing... */ & AnnotationSlice;

// Add to create() call
...createAnnotationSlice(...args),
```

Add message handler in the panel's message listener (file depends on Phase 1-3 structure, likely `packages/extension/src/panel/messageHandler.ts`):
