...
Read the socket path from `/Users/rivermassey/.collaborator/socket-path`, then send newline-delimited JSON.

Call `rpc.discover` to list available methods:
```bash
SOCK=$(cat "/Users/rivermassey/.collaborator/socket-path")
